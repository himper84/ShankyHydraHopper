...
public class GraphicsFunctions
{
    ...
    public string _ScanString(int x1, int y1, int x2, int y2, int color, double range, IntPtr hWin)
    {
        int width = x2 - x1;
        int height = y2 - y1;
        ...
        if (winfunc.WinExists(hWin))
        {
            ...
            for (int bx = 0; bx < bmp.Width; bx++)
            {
                ...
                if (ypixel == 0)
                {
                    if (flag)
                    {
                        xstring = xstring + _LookupChar(ref bmp, xChar, bx - lastx, height, bx);
                        lastx = bx;
                        flag = false;
                    }
    ...
    public string _LookupChar(ref Bitmap bmp, string chr, int w, int h, int x)
    {
        string rv;
        rv = "";
        string[] xlines;
        ...
        if (chr.IndexOf("1") > 0)
        {
            ...
            if (lines.Count > 0)
            {
                ...
                rv = FileFunctions.IniRead(scrapeini, "Chars", chr, "");
            }
            else
            ...
            if (dataentry == 1 && rv == "")
            {
                ...
                if (ScanInputBox(ref bmp, x, chr, "OCR Self-Correction", "What character is referenced here?", ref value) == DialogResult.OK)
                {
                    ...
                    {
                        FileFunctions.IniWrite(scrapeini, "Chars", chr, value);
                        rv = value;
                    }
                }
            }
